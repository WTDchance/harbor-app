...
// âââââ Environment âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââââ Clients âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY)
const anthropic = new Anthropic({ apiKey: ANTHROPIC_API_KEY }) // kept for crisis detection (Sonnet)
const genai = GEMINI_API_KEY ? new GoogleGenAI({ apiKey: GEMINI_API_KEY }) : null

// âââââ Model selection ââââââââââââââââââââââââââââââââââââââââââââââââââ
...
;(async () => {
  if (useGemini) {
    ...
    console.log(`â â ï¸  No GEMINI_API_KEY â falling back to Haiku (slower)`)
    try {
...
// âââââ Connection pre-warming âââââââââââââââââââââââââââââââââââââââââââ
...
// âââââ Practice cache âââââââââââââââââââââââââââââââââââââââââââââââââ
...
async function getCachedPractices(): Promise<any[]> {
  ...
  try {
    ...
    console.warn('â â ï¸  Practice cache refresh failed:', err)
    return practiceCache
  }
}
...
// âââââ Session tracking âââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââââ Express app âââââââââââââââââââââââââââââââââââââââââââââââââââ
const app = express()
...
app.post('/twiml', async (req, res) => {
  ...
  try {
    ...
    console.warn('â â ï¸  Greeting lookup failed:', err)
  }
...
// âââââ WebSocket ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
wss.on('connection', async (ws: WebSocket, req) => {
  ...
  console.log(`ð¡ WebSocket connected | caller: ${callerPhone} | called: ${calledNumber} | remoteAddress: ${req.socket.remoteAddress}`)
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'interrupt':
          ...
          break
        case 'dtmf':
          console.log(`âï¸  DTMF: ${message.digit} (${sessionId})`)
          break
        default:
          console.log(`â ï¸  Unknown: ${message.type}`)
      }
    } catch (error) {
...
// âââââ Handlers âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
async function handleSetup(
  ...
  console.log(`âï¸  Setup: ${callSid}`)

  let practiceId: string | null = null
  let practiceConfig: PracticeConfig | null = null

  if (calledNumber) {
    console.log(`ð Phone matching: calledNumber="${calledNumber}" | callerPhone="${callerPhone}"`)

    const digits = calledNumber.replace(/\D/g, '').slice(-10)
    console.log(`ð Extracted digits for matching: "${digits}" (last 10 digits)`)

    const practices = await getCachedPractices()
    console.log(`ð Found ${practices.length} practices in cache`)
    ...
    practices.forEach((p: any, idx: number) => {
      ...
      console.log(`  [${idx}] "${p.name}" | phone="${p.phone_number}" | digits="${practiceDigits}"${isMatch ? ' â MATCH' : ''}`)
    })
    ...
    if (match) {
      console.log(`â Phone match found: "${match.name}" (id: ${match.id})`)
      practiceId = match.id
      ...
      console.log(`â No phone match found for digits: "${digits}"`)
    }
  } else {
    console.log(`â ï¸  No calledNumber provided for phone matching`)
  }
  ...
  if (practiceId) {
    try {
      ...
      if (cancelledAppts?.length) {
        ...
        console.log(`ð ${cancelledAppts.length} recent openings loaded for AI`)
      }
    } catch (err) {
  ...
  console.log(
    `ð Provider: ${PROVIDER} | Model: ${VOICE_MODEL} | prompt: ${systemPrompt.length} chars`
  )
}
...
async function handlePrompt(ws: WebSocket, message: any, sessionId: string) {
  ...
  console.log(
    `ð£ï¸  Caller: "${utterance}" (${sessionId}) [${session.messages.length} msgs]`
  )
  ...
  // âââââ Crisis check âââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // âââââ Normal conversation (streamed for lowest latency) ââââââââââââââ
  try {
    ...
    console.log(
      `ð¬ ${session.practiceConfig?.ai_name || 'Harbor'}: "${response.substring(0, 100)}..."`
    )
  } catch (err) {
...
function handleInterrupt(sessionId: string, message: any) {
  ...
  console.log(`â¸ï¸  Interrupted (${sessionId})`)
...
// âââââ â¨ Structured data extracted from a call transcript â¨ âââââ
  ...
  try {
    if (session.practiceId) {
      ...
      if (insertError) {
        ...
        console.log(`â Call logged (${inserted.id})`)
      ...
      console.error(`ð UNMATCHED CALL - Cannot insert to call_logs (practice_id is null)`)
...
// âââââ Post-call: extract structured data, upsert patient, link to call log âââââ
async function extractAndLinkPatient(callLogId: string, session: CallSession) {
    ...
    try {
      ...
      if (existingPatient) {
        ...
        if (Object.keys(patientUpdate).length > 0) {
          ...
          console.log(`â Updated patient ${existingPatient.id} with new call data`)
        }
        ...
        if (patientError) {
          ...
          console.log(
            `â New patient created: ${extracted.first_name} ${extracted.last_name || ''} (${newPatient.id})`
          )
        }
      }
    } catch (err) {
  ...
  if (updateError) {
    ...
    console.log(
      `â Call ${callLogId}: extracted data saved (type: ${extracted.call_type}, name: ${extracted.caller_name || 'unknown'})`
    )
  }
}

// âââââ Gemini / Anthropic LLM helpers ââââââââââââââââââââââââââââââââââ
...
// âââââ LLM streaming (primary path for all normal conversation) ââââââââââ
...
async function streamLLMResponse(
  ...
  try {
    if (useGemini && genai) {
      // âââââ Gemini Flash streaming path âââââââââââââââââââââââââââââ
      ...
      // âââââ Anthropic Haiku fallback âââââââââââââââââââââââââââââââ
    ...
    console.log(
      `â ${PROVIDER} stream: TTFB=${ttfb}ms total=${totalMs}ms | len=${fullText.length} | history=${trimmed.length}`
    )
    ...
    console.error(
      `â ${PROVIDER} stream error (${latency}ms):`,
...
async function getLLMResponse(session: CallSession, utterance: string): Promise<string> {
  ...
  try {
    ...
    const latency = Date.now() - t0
    console.log(
      `â ${PROVIDER} in ${latency}ms | len=${text.length} | history=${trimmed.length}`
    )
...
// âââââ Send to ConversationRelay ââââââââââââââââââââââââââââââââââââ
function sendText(ws: WebSocket, text: string) {
  if (ws.readyState !== WebSocket.OPEN) return
  ws.send(JSON.stringify({ type: 'text', token: text, last: true }))
}

// âââââ Crisis alerting âââââââââââââââââââââââââââââââââââââââââââââ
...
// âââââ Start âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
server.listen(PORT, () => {
  console.log(`
...
async function handleSetup(
  ...
  if (practiceId) {
    try {
      ...
      if (cancelledAppts?.length) {
        practiceConfig.available_openings = cancelledAppts.map(a => ({
          date: new Date(a.appointment_date).toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' }),
          time: a.appointment_time || 'TBD',
          type: practiceConfig?.telehealth ? 'telehealth or in-person' : 'in-person',
        }))
        console.log(`ð ${cancelledAppts.length} recent openings loaded for AI`)
...
async function handlePrompt(ws: WebSocket, message: any, sessionId: string) {
  ...
  // Ã¢ÂÂÃ¢ÂÂ Normal conversation (streamed for lowest latency) Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
  try {
    const response = await streamLLMResponse(ws, session, utterance)
    session.transcript.push(`${session.practiceConfig?.ai_name || 'Harbor'}: ${response}`)
...
// ââ Structured data extracted from a call transcript ââ
interface ExtractedCallData {
  caller_name: string | null
  first_name: string | null
  last_name: string | null
...
async function extractCallData(transcript: string[], practiceConfig: PracticeConfig | null): Promise<ExtractedCallData | null> {
  const fullTranscript = transcript.join('\n')
  if (!fullTranscript || fullTranscript.length < 20) return null

  const aiName = practiceConfig?.ai_name || 'Harbor'
  const practiceName = practiceConfig?.practice_name || 'the practice'
  const prompt = `You are analyzing a phone call handled by ${aiName}, the AI receptionist for ${practiceName} (a therapy practice).

Extract structured data from this call transcript. Return ONLY valid JSON (no markdown, no backticks, no explanation).
...
// ââ Post-call: extract structured data, upsert patient, link to call log ââ
async function extractAndLinkPatient(callLogId: string, session: CallSession) {
  const extracted = await extractCallData(session.transcript, session.practiceConfig)
  if (!extracted) return
    ...
    try {
      ...
      if (existingPatient) {
        ...
        if (patientError) {
          ...
          callUpdate.patient_id = newPatient.id
          console.log(`â New patient created: ${extracted.first_name} ${extracted.last_name || ''} (${newPatient.id})`)
        }
      }
    } catch (err) {
      console.error('Patient upsert failed:', err)
  ...
  if (updateError) {
    ...
    console.log(`â Call ${callLogId}: extracted data saved (type: ${extracted.call_type}, name: ${extracted.caller_name || 'unknown'})`)
  }
}
...
async function streamLLMResponse(ws: WebSocket, session: CallSession, utterance: string): Promise<string> {
  ...
  try {
    if (useGemini && genai) {
      ...
      // Ã¢ÂÂÃ¢ÂÂ Anthropic Haiku fallback Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂÃ¢ÂÂ
      const stream = anthropic.messages.stream({
        model: VOICE_MODEL,
        max_tokens: 150,
        system: [
