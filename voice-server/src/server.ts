...
function getGeminiText(result: any): string {
  ...
  if (candidate) {
    ...
    // Maybe text is directly on candidate
    if (typeof candidate?.text === 'string') return candidate.text
    // Log structure for debugging
    console.warn('⚠️ Candidate keys:', Object.keys(candidate || {}))
    if (candidate?.content) console.warn('  content keys:', Object.keys(candidate.content || {}))
    if (Array.isArray(parts)) console.warn('  parts[0] keys:', Object.keys(parts[0] || {}))
  }
  // 4. Deep search: look for any .text string in the response tree
  try {
    const json = JSON.stringify(result)
    const match = json?.match(/"text":"([^"]*)"/)
    if (match && match[1]) {
      console.log('  Found text via JSON search:', match[1].substring(0, 100))
      return match[1]
    }
  } catch(e) { /* ignore */ }
  console.warn('⚠️ No text found in Gemini response. Keys:', Object.keys(result || {}).join(', '))
  return ''
}
