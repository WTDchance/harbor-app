...
async function getPractice() {
  ...
  if (!user) return null
  const { data } = await supabaseAdmin.from('practices').select('id, name').eq('notification_email', user.email).single()
  return data
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const message = `Harbor: Hi ${patient_name}, reminder: you have a ${session_type || 'therapy'} appointment at ${practiceData?.name} on ${timeFormatted}. Reply CONFIRM or CANCEL. For in-person visits, reply HERE when you arrive and we'll notify your therapist! — Harbor`
