...
export async function GET(req: NextRequest) {
    try {
          ...
          if (week) {
                  const end = new Date(week + 'T00:00:00Z'); end.setDate(end.getDate() + 6)
                  query = query.gte('appointment_date', week).lte('appointment_date', end.toISOString().split('T')[0])
                } else {
...
export async function POST(req: NextRequest) {
    try {
          ...
          const { data, error } = await supabaseAdmin.from('appointments').insert({ practice_id: practice.id, source: 'manual', ...body }).select().single()
          if (error) throw error
...
export async function PATCH(req: NextRequest) {
    try {
          ...
          const { data, error } = await supabaseAdmin.from('appointments').update({ ...updates, updated_at: new Date().toISOString() }).eq('id', id).eq('practice_id', practice.id).select().single()
          if (error) throw error
          return NextResponse.json({ appointment: data })
        } catch (e: any) { return NextResponse.json({ error: e.message }, { status: 500 }) }
  }
